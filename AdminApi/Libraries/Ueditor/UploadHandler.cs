            ...
            if (UploadConfig.Base64)
            {
                ...
                try
                {
                    ...
                    if (upRemote)
                    {
                        ...
                        var upload = oss.FileUpload(localPath, "Upload/" + DateTime.UtcNow.ToString("yyyy/MM/dd"), Path.GetFileName(localPath));

                        if (upload)
                        {
                            Common.IO.IOHelper.DeleteFile(localPath);

                            Result.Url = "Upload/" + DateTime.UtcNow.ToString("yyyy/MM/dd") + "/" + Path.GetFileName(localPath);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (upRemote)
                        {
                            //将文件转存至 oss 并清理本地文件
                            var oss = new Common.AliYun.OssHelper();
                            var upload = oss.FileUpload(localPath, "Upload/" + DateTime.UtcNow.ToString("yyyy/MM/dd"), file.FileName);
