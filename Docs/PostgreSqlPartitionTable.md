...
4. TaskService 定时保证当前范围和一个后续范围可写
...
- TaskService 默认每 10 分钟检查一次，因此首版允许的最小单位是小时
...
`Repository.Tool` 已启用 `UseNetEnginePostgreSqlMigrations()`，应继续使用该项目创建 Migration：
...
生成的 `CreateTable` 操作应包含 `NetEngine:Partition*` Annotation。建议在执行前生成 SQL：
...
TaskService 内置以下定时任务：

```text
名称：Database.EnsurePartitions
默认 Cron：0 0/10 * * * ?
```

该任务不会在 TaskService 启动时立即执行，新发现时也默认不启用：

- Debug 模式通过 TaskService 控制台选择启用
- 非 Debug 模式通过管理后台任务配置或对应 Operations 接口启用
- 具体启用方式见 [TaskService](TaskService.md)

生产环境启用流量前，应确认该任务已经启用且至少成功执行过一次，尤其是执行提前生成的 Migration SQL 文件时
...
如果 SQL 文件在生成后跨越了一个或多个分区周期才执行，脚本中的初始分区可能已经过期。此时必须在开放写入前让 `Database.EnsurePartitions` 成功补齐当前和下一范围

系统不创建 `DEFAULT` 分区。缺少匹配范围时，PostgreSQL 会明确拒绝写入，避免数据长期堆积在一个无法有效裁剪的兜底分区中

## TaskService 维护行为

每次执行 `Database.EnsurePartitions` 时会：
...
6. 保证当前雪花 ID 有可写分区
7. 保证当前分区之后至少存在一个连续子分区

如果 TaskService 停机跨越多个周期，任务会从数据库实际最右上界按当前 Attribute 策略向前补齐。已有分区可以使用不同粒度，但从当前写入范围到预建范围必须连续；维护任务不会修复最右边界之前的历史空洞

单张父表一次最多创建 32 个子分区。超过安全上限时，该父表在本轮不会创建任何分区，需要人工确认停机跨度和目标粒度后先补充部分范围，再恢复自动维护

维护只向前创建，不会自动补充早于最老已有分区的范围，也不会删除历史分区
...
1. 停止或禁用旧版本 TaskService 的 `Database.EnsurePartitions`
2. 修改实体 Attribute
3. 创建并审核 Migration
4. 部署使用新模型的应用和 TaskService
5. 启用新版本维护任务并确认执行成功

不要让使用不同分区间隔的新旧 TaskService 同时运行。分布式锁只能让它们串行执行，不能判断哪一个版本的策略更新

## Migration 回滚注意事项

- 回滚首次创建分区表的 Migration 会删除父表及其子分区，表内数据也会随之删除
- 回滚仅修改 `IntervalHours` 的 Migration 不会重建已有子分区
- 回滚策略 Migration 时，应同时回滚实体 Attribute 和 TaskService 版本，避免代码模型与 Migration 版本不一致
...
```sql
SELECT "Name", "Cron", "IsEnable"
FROM "TaskSetting"
WHERE "Category" = 'ScheduleTask'
  AND "Name" = 'Database.EnsurePartitions';
```

应通过管理后台或 TaskService 现有管理接口修改启用状态，不建议直接更新任务配置表

## 常见问题排查

| 现象 | 常见原因 | 处理方式 |
|---|---|---|
| `no partition of relation ... found for row` | 当前 ID 没有匹配范围 | 检查维护任务状态、子分区边界和写入 ID 时间 |
...
| 本次需要创建数量超过 32 | TaskService 停机时间过长或间隔过小 | 人工审核并先补充部分连续范围，再恢复维护 |
| 子分区名称已存在但范围不一致 | 同名表或人工分区与期望冲突 | 核对对象所属父表和实际范围，不要让任务自动覆盖 |
| 分布式锁续期失败 | Redis 不可用、网络异常或维护耗时异常 | 恢复 Redis，检查数据库 DDL 阻塞后重新执行任务 |
...
`DEFAULT` 分区会让缺少正式范围的问题不再立即失败，数据可能持续进入兜底表并降低裁剪和维护效果。当前选择明确拒绝越界写入，并通过定时任务提前创建下一范围

### TaskService 启动时会立即创建分区吗

不会。分区维护只作为标准定时任务运行，必须按 TaskService 现有流程显式启用
...
| `Repository/Migrations` | Migration Annotation 和 SQL 生成扩展 |
| `Repository/Partitioning` | 模型校验、边界计算与运行维护 |
| `Infrastructure/IdentifierGenerator/SnowflakeIdLayout.cs` | 雪花 ID 持久化布局和时间换算 |
| `Presentation/TaskService/Tasks/DatabaseMaintenanceTask.cs` | 定时任务入口 |
