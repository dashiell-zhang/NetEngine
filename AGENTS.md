...
本项目是框架模板，修改以当前模板的清晰、一致和正确为目标，不需要考虑历史版本、旧接口、旧配置或旧缓存数据的兼容。除非用户明确要求，不为历史兼容新增版本标记、兼容分支、双读双写、迁移或回退逻辑；涉及契约调整时，同步更新仓库内当前调用方和文档
...
- 任务范围内可直接调整路由、字段、状态码和序列化行为，无需保留历史接口兼容；同步更新仓库内受影响的调用方和文档，不做无关契约变更
